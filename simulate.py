...
def simulate_with_controls(
    ...
    # ===============================================================
# BLOQUE 2 — Interfaz Streamlit + Optimización de fechas
# ===============================================================
...
# ---------------------------------------------------------------
st.set_page_config(page_title="WeedCropSystem v3.13.1", layout="wide")
st.title("🌾 WeedCropSystem v3.13.1 — AUC + Optimización de fechas")
...
if mode == "Simulación única":
  ...
  if st.sidebar.button("▶ Ejecutar simulación"):
    ...
    if df is None or df.empty:
      ...
      # Gráfico adicional de competencia
      fig2 = go.Figure()
      ...
      st.download_button("📥 Descargar CSV", df.to_csv(index=False).encode(), "simulacion_v3131.csv","text/csv")

# ---------------------------------------------------------------
# 🧠 OPTIMIZADOR DE FECHAS
# ---------------------------------------------------------------
else:
  st.sidebar.header("Espacio de búsqueda")
  preR_enable = st.sidebar.checkbox("Optimizar presiembra", True)
  preemR_enable = st.sidebar.checkbox("Optimizar preemergente", True)
  postR_enable = st.sidebar.checkbox("Optimizar postresidual", True)
  gram_enable = st.sidebar.checkbox("Optimizar graminicida", True)
  ...
  def _cand_preR():
    return [sow_date - dt.timedelta(days=d) for d in range(30, 13, -int(step_preR))]
  def _cand_preem():
  ...
  if run_opt:
    ...
    for i,(d_preR, d_preem, d_postR, d_gram) in enumerate(combos, 1):
      ...
      if i % max(1, len(combos)//100) == 0: prog.progress(min(1.0, i/len(combos)))
    prog.progress(1.0)
    ...
    if not results:
      ...
      st.download_button("📥 Descargar resultados CSV", res_df.to_csv(index=False).encode(), "opt_fechas_resultados.csv","text/csv")
